    ...
    public class BookServiceImpl implements BookService {
        ...
        public ResponseCommon<AddBookResponse> addBook(AddBookRequest addBookRequest, MultipartFile file) {
            try {
                ...
                Author author = authorRepository.findAuthorByNameAuthor(addBookRequest.getAuthor().getNameAuthor()).orElse(null);
                if (author == null) {
                    log.debug("Add Book failed: Category does not exist");
                    return new ResponseCommon<>(ResponseCode.CATEGORY_NOT_EXIST, null);
                }
                book.setAuthors(Set.of(author));
                Category category = categoryRepository.findCategoryByNameCategory(addBookRequest.getCategory().getNameCategory()).orElse(null);
                if (category == null) {
                    log.debug("Add Book failed: Category does not exist");
                    return new ResponseCommon<>(ResponseCode.CATEGORY_NOT_EXIST, null);
                }
        ...
        public ResponseCommon<UpdateBookResponse> updateBook(UpdateBookRequest updateBookRequest, MultipartFile file) {
            try {
                ...
                if (Objects.isNull(bookExist)) {
                    ...
                    Category category = categoryRepository.findCategoryByNameCategory(updateBookRequest.getCategory().getNameCategory()).orElse(null);
                    ...
                    Author author = authorRepository.findAuthorByNameAuthor(updateBookRequest.getAuthor().getNameAuthor()).orElse(null);
                    if (author == null) {
